...
const matchJobsToCleaners = (cleaners, jobs) => {
  ...
  const availableRegions = useMemo(() => {
    ...
    console.log('🗺️ Available regions:', sortedRegions);
  ...
  // --- STATS CALCULATION ---
  useEffect(() => {
    console.log('📊 Calculating stats for view:', view);
    
    // Get unique cleaners (excluding unassigned)
    ...
    console.log('👥 Unique cleaners:', uniqueCleaners.size);

    // Determine which dates to scan based on view
    let datesToScan = [];
    if (view === 'daily') {
      ...
      datesToScan = monthDays.filter(Boolean);
    }
    
    console.log('📅 Dates to scan:', datesToScan.length);
  ...
  }, [view, selectedDay, currentWeek, currentMonth, availabilityData, selectedRegion, weekDates, monthDays]);
  ...
  const getCleanersForSlot = (date, blockIdOrHour) => {
    ...
    console.log('🔍 Getting cleaners for:', {
    ...
    // Filter to cleaners for this specific week
    ...
    console.log('👥 Filtered cleaners for week:', filtered.length);
    ...
    console.log('✅ Available:', available.length, '🔴 Booked:', booked.length);
  ...
  const getAvailableCleanersForDay = (date) => {
    ...
    const cleanersForThisWeek = availabilityData.filter(
      c => c.weekStarting === weekString && c.id !== 'unassigned'
    );
  ...
  const openSlotDetails = (date, blockIdOrHour) => {
    ...
    setSelectedSlot({
      ...
      date: date,
      block: isHourly 
        ? { label: blockIdOrHour, time: blockIdOrHour } 
        : TIME_BLOCKS.find(b => b.id === blockIdOrHour),
      available, 
      booked, 
      isHourly
    });
  ...
  const renderDatePicker = () => {
    ...
    return createElement('div', { 
      className: 'absolute top-full mt-2 w-72 bg-white border border-gray-200 rounded-lg shadow-lg z-20 p-4' 
    },
      createElement('div', { className: 'flex items-center justify-between mb-3' },
        ...
        createElement('div', { className: 'font-semibold text-sm' }, 
      ...
      createElement('div', { className: 'grid grid-cols-7 gap-1 text-center text-xs text-gray-500' }, 
        ...['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => 
          createElement('div', { key: i, className: 'p-1' }, d)
        )
      ),
  ...
  if (loading && availabilityData.length === 0) {
    return createElement('div', { className: 'min-h-screen bg-gray-50 flex items-center justify-center' },
      createElement('div', { className: 'text-center' },
        createElement('div', { className: 'text-4xl mb-4' }, '🧹'),
        createElement('div', { className: 'text-xl font-semibold text-gray-700' }, 'Loading Calendar...'),
        createElement('div', { className: 'mt-2 text-sm text-gray-500' }, 'Fetching availability data')
      )
    );
  }
  ...
  return createElement('div', { className: 'min-h-screen bg-gray-50 p-2 md:p-5' },
    ...
    networkError && createElement('div', { 
      className: 'max-w-7xl mx-auto mb-4 p-4 bg-red-100 text-red-800 rounded-lg border border-red-300 flex items-center gap-3' 
    },
      createElement('span', { className: 'text-2xl' }, '⚠️'),
      createElement('div', null,
        createElement('div', { className: 'font-semibold' }, 'Connection Error'),
        createElement('div', { className: 'text-sm' }, networkError)
      )
    ),
    ...
    createElement('div', { className: 'max-w-7xl mx-auto mb-4 bg-white rounded-xl shadow-sm p-4 sm:p-5 flex items-center justify-between' },
      createElement('div', { className: 'flex items-center gap-3' },
        ...
        createElement('div', null,
          ...
          lastSync && createElement('p', { className: 'text-xs text-gray-500' }, 
    ...
    createElement('div', { className: 'max-w-7xl mx-auto mb-4 grid grid-cols-2 lg:grid-cols-4 gap-4' },
      createElement('div', { 
        ...
        onClick: () => setShowCleanersModal(true) 
      },
      ...
      createElement('div', { className: 'bg-white rounded-xl shadow-sm p-3 sm:p-4' },
        createElement('div', { className: 'flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4' },
            ...
            createElement('div', { className: 'flex gap-2 flex-wrap' },
              availableRegions.map(region => {
                ...
                return createElement('button', {
                  ...
                  className: `px-3 py-2.5 sm:px-4 rounded-lg font-medium text-xs sm:text-sm transition-colors ${
          ...
          createElement('div', { className: 'flex items-center gap-2 relative self-center' },
            ...
            createElement('button', { 
              onClick: () => { 
                let initialDate = view === 'daily' ? selectedDay : view === 'weekly' ? currentWeek : currentMonth; 
    ...
    createElement('div', { className: 'max-w-7xl mx-auto' },
      view === 'weekly' && createElement(WeeklyView, { 
        weekDates, 
        getCleanersForSlot, 
        openSlotDetails, 
        setSelectedDay, 
      ...
      view === 'daily' && createElement(DailyView, { 
        availabilityData, 
        selectedDay, 
        selectedRegion, 
        searchQuery, 
      ...
      view === 'monthly' && createElement(MonthlyView, { 
        monthDays, 
        getAvailableCleanersForDay, 
        setSelectedDay, 
        setView 
      })
    ),
