...
const DustBustersCalendar = () => {
  ...
  const getAvailableCleanersForDay = (date) => {
    ...
    const cleanersForWeek = filteredCleaners.filter(c => c.weekStarting ? c.weekStarting === weekString : true);
    
    cleanersForWeek.forEach(c => {
        for (const h of hourlySlots) {
            if (c[`${dayPrefix}_${h}`] === 'AVAILABLE') {
                if (!available.has(c.id)) available.set(c.id, c);
                break;
            }
        }
    });
  ...
  const availableRegions = useMemo(() => {
    ...
    availabilityData.forEach(c => {
      if (c.region?.trim()) r.add(c.region);
      if (Array.isArray(c.regions)) c.regions.forEach(reg => { if (reg?.trim()) r.add(reg); });
    });
    return ['all', ...Array.from(r).sort()];
  }, [availabilityData]);

  const getRegionColor = (r) => {
    const l = r?.toLowerCase(), s = { 'all': 'teal', 'charlotte': 'yellow', 'raleigh': 'stone', 'triad': 'purple' };
    if (s[l]) return s[l];
    const f = ['pink', 'indigo', 'cyan', 'lime', 'orange'];
    let h = 0;
    if (l) for (let i = 0; i < l.length; i++) h = l.charCodeAt(i) + ((h << 5) - h);
    return f[Math.abs(h % f.length)];
  };

  const getRegionEmoji = (r) => ({ 'Charlotte': '🟡', 'Triad': '🟣', 'Raleigh': '🟤', 'Asheville': '⛰️', 'Wilmington': '🌊', 'Durham': '🐂' }[r] || '📍');
  
  // --- NEW: SMART SEARCH NAVIGATION LOGIC ---
  ...
  return React.createElement('div', { className: 'min-h-screen bg-gray-50 p-5' },
    ...
    React.createElement('div', { className: 'max-w-7xl mx-auto mb-5' },
      React.createElement('div', { className: 'bg-white rounded-xl shadow-sm p-5 flex items-center justify-between' },
        ...
        React.createElement('button', { onClick: loadAvailabilityData, disabled: loading, className: 'flex items-center gap-2 px-4 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600 transition-colors disabled:opacity-50' }, loading ? '↻ Refreshing...' : '↻ Refresh')
      )
    ),
    // AI Banner
    React.createElement('div', { className: 'max-w-7xl mx-auto mb-5' },
    ...
    React.createElement('div', { className: 'max-w-7xl mx-auto mb-5' },
      React.createElement('div', { className: 'grid grid-cols-1 md:grid-cols-4 gap-4' },
        React.createElement('div', { className: 'bg-white rounded-xl shadow-sm p-5 cursor-pointer hover:bg-gray-50 transition-colors', onClick: () => { setCleanerModalRegionFilter('all'); setShowCleanersModal(true); } },
      ...
      React.createElement('div', { className: 'bg-white rounded-xl shadow-sm p-5' },
        React.createElement('div', { className: 'relative' },
          ...
          React.createElement('input', { 
            ...
            onKeyDown: handleSmartSearch, // <-- THIS IS THE NEW PART
        ...
        React.createElement('div', { className: 'bg-white rounded-xl shadow-sm p-4' }, 
            ...
            React.createElement('div', { className: 'flex items-center justify-between' }, 
                React.createElement('div', { className: 'flex gap-3 flex-wrap' }, 
                    availableRegions.map(region => { const color = getRegionColor(region), emoji = region === 'all' ? '' : getRegionEmoji(region.charAt(0).toUpperCase() + region.slice(1)), label = region === 'all' ? 'All Regions' : `${emoji} ${region.charAt(0).toUpperCase() + region.slice(1)}`; return React.createElement('button', { key: region, onClick: () => setSelectedRegion(region.toLowerCase()), className: `px-6 py-2.5 rounded-lg font-medium text-sm transition-colors ${selectedRegion === region.toLowerCase() ? `bg-${color}-500 text-white` : `bg-${color}-50 text-${color}-700 hover:bg-${color}-100`}` }, label); })
                ), 
                React.createElement('div', { className: 'flex items-center gap-2 relative' }, 
                    React.createElement('button', { onClick: () => { if (view === 'daily') setSelectedDay(new Date(selectedDay.getTime() - 86400000)); else if (view === 'weekly') setCurrentWeek(new Date(currentWeek.getTime() - 604800000)); else setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() - 1)); }, className: 'px-3 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600' }, '←'), 
                    React.createElement('button', { onClick: () => { let initialDate; if (view === 'daily') initialDate = selectedDay; else if (view === 'weekly') initialDate = currentWeek; else initialDate = currentMonth; setDatePickerMonth(initialDate); setShowDatePicker(!showDatePicker); }, className: 'px-4 py-2 font-semibold text-gray-800 min-w-[250px] text-center text-sm cursor-pointer hover:bg-gray-100 rounded-md' }, view === 'daily' ? selectedDay.toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric', year: 'numeric' }) : view === 'weekly' ? `Week of ${weekDates[0].toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}` : currentMonth.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })), 
                    React.createElement('button', { onClick: () => { const today = new Date(); setSelectedDay(today); setCurrentWeek(getMonday(today)); setCurrentMonth(today); }, className: 'px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 text-sm font-medium' }, 'Today'), 
                    React.createElement('button', { onClick: () => { if (view === 'daily') setSelectedDay(new Date(selectedDay.getTime() + 86400000)); else if (view === 'weekly') setCurrentWeek(new Date(currentWeek.getTime() + 604800000)); else setCurrentMonth(new Date(currentMonth.getFullYear(), currentMonth.getMonth() + 1)); }, className: 'px-3 py-2 bg-blue-500 text-white rounded-lg hover:bg-blue-600' }, '→'), 
    ...
    // Calendar Views and Modals
    view === 'daily' && React.createElement('div', { /* ... daily view JSX ... */ }),
    view === 'weekly' && React.createElement('div', { /* ... weekly view JSX ... */ }),
    view === 'monthly' && React.createElement('div', { /* ... monthly view JSX ... */ }),
