...
const DustBustersCalendar = () => {
  ...
  const renderDailyView = () => {
    ...
    return React.createElement('div', {
        ...
        style: { gridTemplateColumns: `150px repeat(${allItemsForDay.length}, 1fr)` }
      },
        ...
        ...allItemsForDay.map(c => {
          ...
          if (c.hasSchedule === false && c.job) {
            if (c.job.date === selectedDay.toISOString().split('T')[0] && c.job.slots.includes(hour)) {
              isBooked = true;
              bookingDetails = {
                jobNumber: c.job.jobNumber, customer: c.job.customer
              };
            }
  ...
  // Paste your full render function from the last working code here
  return React.createElement(/* ... */);
};

// Paste the root render call from your last working code here
