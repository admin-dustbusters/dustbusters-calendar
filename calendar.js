...
const DustBustersCalendar = () => {
  ...
  const handleSmartSearch = (e) => { if (e.key === 'Enter') { e.preventDefault(); const query = searchQuery.trim(); if (!query) return; const parsedDate = new Date(query); if (!isNaN(parsedDate.getTime()) && (query.match(/[a-zA-Z]/) || query.match(/\//) || query.match(/-/))) { setSelectedDay(parsedDate); setCurrentWeek(getMonday(parsedDate)); setCurrentMonth(parsedDate); setView('daily'); return; } const monthNames = ["jan", "feb", "mar", "apr", "may", "jun", "jul", "aug", "sep", "oct", "nov", "dec"]; const monthIndex = monthNames.findIndex(m => query.toLowerCase().startsWith(m)); if (monthIndex !== -1) { const newMonth = new Date(new Date().getFullYear(), monthIndex, 1); setCurrentMonth(newMonth); setView('monthly'); } } };
  ...
  return React.createElement('div', { className: 'min-h-screen bg-gray-50 p-5' },
    // All UI elements are built from here using React.createElement, preserving your original syntax
    // Header
    ...
    React.createElement('div', { className: 'max-w-7xl mx-auto mb-5' }, React.createElement('div', { className: 'grid grid-cols-4 gap-4' }, React.createElement('div', { className: 'bg-white rounded-xl shadow-sm p-5 cursor-pointer hover:bg-gray-50 transition-colors', onClick: () => { setCleanerModalRegionFilter('all'); setShowCleanersModal(true); } }, React.createElement('div', { className: 'text-xs font-semibold text-gray-500 uppercase mb-2' }, 'Total Cleaners'), React.createElement('div', { className: 'text-3xl font-bold text-gray-800' }, dynamicStats.totalCleaners)), React.createElement('div', { className: 'bg-white rounded-xl shadow-sm p-5 cursor-pointer hover:bg-gray-50 transition-colors', onClick: () => setShowAvailableModal(true) }, React.createElement('div', { className: 'text-xs font-semibold text-gray-500 uppercase mb-2' }, availableLabel), React.createElement('div', { className: 'text-3xl font-bold text-gray-800' }, dynamicStats.cleanersAvailable)), React.createElement('div', { className: 'bg-white rounded-xl shadow-sm p-5 cursor-pointer hover:bg-gray-50 transition-colors', onClick: () => setShowBookedModal(true) }, React.createElement('div', { className: 'text-xs font-semibold text-gray-500 uppercase mb-2' }, 'Booked Slots'), React.createElement('div', { className: 'text-3xl font-bold text-gray-800' }, dynamicStats.bookedSlots)), React.createElement('div', { className: 'bg-white rounded-xl shadow-sm p-5 cursor-pointer hover:bg-gray-50 transition-colors', onClick: () => setShowOpenSlotsModal(true) }, React.createElement('div', { className: 'text-xs font-semibold text-gray-500 uppercase mb-2' }, 'Open Slots'), React.createElement('div', { className: 'text-3xl font-bold text-gray-800' }, dynamicStats.openSlots)))),
