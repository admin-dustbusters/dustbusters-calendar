...
const DustBustersCalendar = () => {
  ...
  useEffect(() => {
      // Stats calculation logic remains as it was in your working version
  ...
  const renderCleanersModal = () => {
    ...
    availabilityData.forEach(c => {
      if (!uniqueCleanersMap.has(c.id || c.name)) {
        uniqueCleanersMap.set(c.id || c.name, c);
      }
    });
    ...
    return React.createElement('div', {
          ...
          React.createElement('div', { key: region, className: 'mb-8' },
            ...
            React.createElement('div', { className: 'grid grid-cols-1 md:grid-cols-2 gap-4' },
              ...groupedByRegion[region].map(c =>
                React.createElement('div', { key: c.id || c.name, className: 'bg-gray-50 rounded-lg p-4 border border-gray-200' },
      ...
      React.createElement('button', { onClick: () => setShowCleanersModal(false), className: 'mt-6 flex-shrink-0 px-6 py-3 bg-gray-200 text-gray-700 rounded-lg font-semibold hover:bg-gray-300 self-center'}, 'Close')
    ));
  };
